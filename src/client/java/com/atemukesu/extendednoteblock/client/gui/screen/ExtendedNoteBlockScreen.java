 ...
 */
public class ExtendedNoteBlockScreen extends AbstractContainerScreen<ExtendedNoteBlockScreenHandler> {
    private int note;
     ...
     * @param title     屏幕的标题。
     */
    public ExtendedNoteBlockScreen(ExtendedNoteBlockScreenHandler handler, Inventory inventory, Component title) {
    ...
    protected void init() {
        ...
        this.sustainField.setResponder(text -> {
        ...
        this.delayField.setResponder(
                text -> this.delayedPlayingTime = parseInteger(text, 0,
                        ExtendedNoteBlockEntity.MAX_DELAY_MS, this.delayedPlayingTime));
        ...
        this.fadeInField.setResponder(text -> {
        ...
        this.fadeOutField.setResponder(text -> {
        ...
        this.addRenderableWidget(Button.builder(
            ...
            button -> minecraft.setScreen(new AdvancedSettingsScreen(this, this.menu.blockEntity))
        ).bounds(advancedButtonX, advancedButtonY, advBtnWidth, 20).build());
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (this.instrumentComboBox.mouseClicked(event, doubleClick)) {
            return true;
        }
        if (this.pianoWidget.mouseClicked(event, doubleClick)) {
            return true;
        }
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        ...
        if (this.instrumentComboBox.mouseReleased(event)) {
            return true;
        }
        if (this.pianoWidget.mouseReleased(event)) {
            return true;
        }
     ...
     */
    public class PianoWidget extends AbstractWidget {
        ...
        public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
            if (event.button() == 0 && isMouseOverScrollbar(event.x(), event.y())) {
                ...
                this.mouseDragged(event, 0.0, 0.0); // 立即更新位置
                return true;
            }
            return super.mouseClicked(event, doubleClick);
        }
            ...
            if (this.isDraggingScrollbar) {
                ...
                if (contentScrollRange > 0) {
                    ...
                    if (trackWidth > 0) {
                        ...
                        double scrollPercentage = Mth.clamp(handleTargetX / trackWidth, 0.0, 1.0);
        ...
        protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
            ...
            onHover.accept(hoveredKey != -1
                    ? Component.translatable("gui.extendednoteblock.piano.key_info", getNoteName(hoveredKey), hoveredKey)
                    : Component.empty());
        ...
        public void onClick(MouseButtonEvent event, boolean doubleClick) {
            ...
            if (key != -1) {
                ...
                ExtendedNoteBlockScreen.this.onClose(); // 选中音符后直接关闭界面
            }
        }
        ...
        private void clampScroll() {
            scrollOffset = Mth.clamp(scrollOffset, 0, Math.max(0, totalWidth - this.width));
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        // 绘制标题和标签
        context.centeredText(font, this.title, this.width / 2, 15, 0xFFFFFFFF);
        context.text(font, Component.translatable("gui.extendednoteblock.instrument"),
                this.instrumentComboBox.getX(), this.instrumentComboBox.getY() - 10, 0xFFA0A0A0);
        context.text(font, Component.translatable("gui.extendednoteblock.velocity"),
                this.velocityField.getX(), this.velocityField.getY() - 10, 0xFFA0A0A0);
        Component sustainLabel = Component.translatable("gui.extendednoteblock.sustain_ticks");
        context.text(font, sustainLabel, this.sustainField.getX(), this.sustainField.getY() - 10,
                0xFFA0A0A0);
        context.text(font, Component.translatable("gui.extendednoteblock.sustain.info"),
                this.sustainField.getX() + 4, this.sustainField.getY() + this.sustainField.getHeight() + 4,
                0xFF808080);

        Component delayLabel = Component.translatable("gui.extendednoteblock.delay_ms");
        context.text(font, delayLabel, this.delayField.getX(), this.delayField.getY() - 10,
                0xFFA0A0A0);
        // 使用 delayLabel 和 delayField 的坐标来定位提示信息
        context.text(font, Component.translatable("gui.extendednoteblock.delay_ms.info"),
                this.delayField.getX() + 4, this.delayField.getY() + this.delayField.getHeight() + 4, 0xFF808080);
        context.text(font, Component.translatable("gui.extendednoteblock.fadein_time"),
                this.fadeInField.getX(), this.fadeInField.getY() - 10, 0xFFA0A0A0);
        context.text(font, Component.translatable("gui.extendednoteblock.fadein_time.info"),
                this.fadeInField.getX() + 4, this.fadeInField.getY() + this.fadeInField.getHeight() + 4, 0xFF808080);

        // 绘制淡出时间的标签和信息
        context.text(font, Component.translatable("gui.extendednoteblock.fadeout_time"),
                this.fadeOutField.getX(), this.fadeOutField.getY() - 10, 0xFFA0A0A0);
        context.text(font, Component.translatable("gui.extendednoteblock.fadeout_time.info"),
                this.fadeOutField.getX() + 4, this.fadeOutField.getY() + this.fadeOutField.getHeight() + 4, 0xFF808080);

        // 绘制底部中央的悬停提示
        context.centeredText(font, this.hoveredKeyText, this.width / 2, this.height - 20,
                0xFFFFFFFF);
     ...
     * {@inheritDoc}
     * <p>
     * 在这个屏幕中，我们使用 {@link #render(GuiGraphicsExtractor, int, int, float)} 中的
    ...
    private void sendUpdatePacket() {
        try {
            ...
            int finalSustain = Mth.clamp(this.sustain, 0, 400);
            int finalDelayedPlayingTime = Mth.clamp(this.delayedPlayingTime, 0,
                    ExtendedNoteBlockEntity.MAX_DELAY_MS);
            int finalInstrumentId = Mth.clamp(this.instrumentId, 0, 128);
            ...
            ClientPlayNetworking.send(new ModPayloads.UpdateNoteBlockPayload(
                    this.menu.blockPos, finalNote, finalVelocity, finalSustain,
    ...
    public void onClose() {
        sendUpdatePacket();
        ClientPlayNetworking.send(new ModPayloads.PreviewRequestPayload(this.menu.blockPos));
        super.onClose();
    }
