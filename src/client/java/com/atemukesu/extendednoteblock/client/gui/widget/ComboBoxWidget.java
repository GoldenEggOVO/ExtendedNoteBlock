...
public class ComboBoxWidget<T> extends AbstractWidget {
    ...
    public void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.fill(this.getX(), this.getY(), this.getX() + this.getWidth(), this.getY() + this.getHeight(),
                mainBoxColor);
        context.outline(this.getX(), this.getY(), this.getWidth(), this.getHeight(), COLOR_BORDER);
        String displayText = getDisplayText();
        if (!displayText.isEmpty()) {
            String truncatedText = truncateText(textRenderer, displayText, this.getWidth() - 20);
            context.text(textRenderer, truncatedText, this.getX() + 4, this.getY() + (this.height - 8) / 2,
                    COLOR_TEXT_MAIN, false);
        }
        int arrowX = this.getX() + this.getWidth() - 12;
        int arrowY = this.getY() + this.getHeight() / 2;
        context.text(textRenderer, this.isOpen ? "▲" : "▼", arrowX, arrowY - 4, COLOR_TEXT_MAIN, false);
    }

    public void renderDropdownOverlay(GuiGraphicsExtractor context, int mouseX, int mouseY) {
    ...
    private String truncateText(Font textRenderer, String text, int maxWidth) {
        ...
        String ellipsis = "...";
        return textRenderer.plainSubstrByWidth(text, maxWidth - textRenderer.width(ellipsis)) + ellipsis;
    }

    private void renderDropdown(GuiGraphicsExtractor context, int mouseX, int mouseY, Font textRenderer) {
        ...
        context.fill(this.getX(), dropdownY, this.getX() + this.getWidth(), dropdownY + dropdownHeight,
                COLOR_DROPDOWN_BG);
        context.outline(this.getX(), dropdownY, this.getWidth(), dropdownHeight, COLOR_DROPDOWN_BORDER);
        int totalContentHeight = this.options.size() * ITEM_HEIGHT;
        int maxScroll = Math.max(0, totalContentHeight - (visibleItems * ITEM_HEIGHT));
        int scrollOffset = Mth.clamp(this.scroll, 0, maxScroll);
        ...
        for (int i = 0; i < this.options.size(); i++) {
            ...
            context.text(textRenderer, truncatedText, this.getX() + 4, itemY + 2, textColor, false);
        }
    ...
    private void renderScrollbar(GuiGraphicsExtractor context, int dropdownY, int dropdownHeight, int maxScroll) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        if (this.isOpen && this.options.size() > MAX_VISIBLE_ITEMS) {
            ...
            if (mouseX >= this.getX() && mouseX < this.getX() + this.getWidth() &&
                ...
                int totalContentHeight = this.options.size() * ITEM_HEIGHT;
                int maxScroll = Math.max(0, totalContentHeight - (visibleItems * ITEM_HEIGHT));
                this.scroll = Mth.clamp(this.scroll - (int) (verticalAmount * ITEM_HEIGHT), 0, maxScroll);
                return true;
            }
        }
        return false;
    ...
    private void updateScrollFromMousePosition(double mouseY, int dropdownY, int dropdownHeight) {
        ...
        double clampedMouseY = Mth.clamp(relativeMouseY - scrollbarHeight / 2.0, 0.0, scrollableArea);
    ...
    private void updateMessage() {
        if (this.selectedIndex >= 0 && this.selectedIndex < this.options.size()) {
            this.setMessage(Component.nullToEmpty(this.options.get(this.selectedIndex).toString()));
        } else {
