...
public class SoundPackManager {
    ...
    private SoundPackInfo loadPackFromZip(Path zipPath, String id) throws IOException {
        try (FileSystem fs = FileSystems.newFileSystem(zipPath, (ClassLoader) null)) {
            ...
            if (id.startsWith(DEFAULT_PACK_ID)) {
                displayName = Component.translatable("gui.extendednoteblock.pack_manager.default_pack_name").getString();
            }
    ...
    public void setActivePack(String packId) {
        ...
        client.execute(() -> {
            ...
            Collection<String> allAvailablePackNames = resourcePackManager.getAvailableIds();

            List<String> originalEnabledPacks = new ArrayList<>(resourcePackManager.getSelectedIds());
            ...
            if (!new HashSet<>(originalEnabledPacks).equals(new HashSet<>(newEnabledPacks))) {
                resourcePackManager.setSelected(newEnabledPacks);
                ConfigManager.getConfig().activeSoundPackId = this.activePackId;
                ConfigManager.saveConfig();
                client.reloadResourcePacks().whenComplete((v, throwable) -> {
                    if (throwable != null)
    ...
    public boolean isCurrentPackActuallyEnabled() {
        ...
        PackRepository resourcePackManager = client.getResourcePackRepository();
        Collection<String> enabledPacks = resourcePackManager.getSelectedIds();
