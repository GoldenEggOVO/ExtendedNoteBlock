...
public class ClientSoundManager {
    ...
    private static boolean wasGamePaused;
    private static int resumeValidationTicks;
    private static final Set<UUID> ACTIVE_BEFORE_PAUSE = new HashSet<>();

    // 定义一个极小的音量阈值，防止被声音引擎剔除
    // Minecraft 可能会剔除音量 <= 0 的声音
    private static final float MIN_ALIVE_VOLUME = 0.0001f;

    public static void tickPauseRecovery(Minecraft client) {
        boolean paused = client.isPaused();
        if (!wasGamePaused && paused) {
            ACTIVE_BEFORE_PAUSE.clear();
            PLAYING_SOUNDS.forEach((soundId, sound) -> {
                if (!sound.isStopped() && client.getSoundManager().isActive(sound)) {
                    ACTIVE_BEFORE_PAUSE.add(soundId);
                }
            });
        }
        if (wasGamePaused && !paused) {
            client.getSoundManager().resume();
            resumeValidationTicks = 2;
        }
        wasGamePaused = paused;

        if (paused || resumeValidationTicks <= 0) {
            return;
        }
        resumeValidationTicks--;
        if (resumeValidationTicks > 0) {
            return;
        }

        for (UUID soundId : ACTIVE_BEFORE_PAUSE) {
            StoppablePositionalSoundInstance sound = PLAYING_SOUNDS.get(soundId);
            if (sound != null && !sound.isStopped() && !client.getSoundManager().isActive(sound)) {
    ...
    public static void playSound(BlockPos pos, UUID soundId, int instrumentId, int note, int velocity,
        ...
        ResolvedSample sample = resolveSample(activePack, instrumentId, note);
        if (sample == null) {
            return;
        }
        int closestNote = sample.note();
        int playbackInstrumentId = sample.instrumentId();
        ...
        if (playbackInstrumentId == DRUM_KIT_INSTRUMENT_ID) {
            pitch = 1.0f;
        }

        Identifier soundIdentifier = Identifier.fromNamespaceAndPath("extendednoteblock",
                "notes." + playbackInstrumentId + "." + closestNote);
        SoundEvent soundEvent = SoundEvent.createVariableRangeEvent(soundIdentifier);
        ...
        StoppablePositionalSoundInstance soundInstance = new StoppablePositionalSoundInstance(
                soundEvent, SoundSource.RECORDS, safeVolume, pitch, pos);
    ...
    public static void playAdvancedSound(BlockPos pos, UUID soundId, int instrumentId, int note,
        ...
        StoppablePositionalSoundInstance soundInstance = new StoppablePositionalSoundInstance(
                soundEvent, SoundSource.RECORDS, safeVolume, finalStartPitch, pos);
    ...
    private static ResolvedSample resolveSample(SoundPackInfo pack, int instrumentId, int targetNote) {
        int resolvedInstrument = instrumentId;
        List<Integer> notes = pack.availableNotes().get(instrumentId);
        if (notes == null || notes.isEmpty()) {
            resolvedInstrument = 0;
            notes = pack.availableNotes().get(0);
        }
        if (notes == null || notes.isEmpty()) {
            Map.Entry<Integer, List<Integer>> fallback = pack.availableNotes().entrySet().stream()
            ...
            resolvedInstrument = fallback.getKey();
            notes = fallback.getValue();
        }

        int closestNote = notes.getFirst();
        int closestDistance = Math.abs(closestNote - targetNote);
        for (int candidate : notes) {
            int distance = Math.abs(candidate - targetNote);
            if (distance < closestDistance) {
                closestNote = candidate;
                closestDistance = distance;
            }
        }
        return new ResolvedSample(resolvedInstrument, closestNote);
    }

    private record ResolvedSample(int instrumentId, int note) {
    }
}
