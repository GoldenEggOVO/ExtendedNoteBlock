...
Release builds must run the existing source-preparation pipeline before Gradle: `prepare_paper_custom_model_data.py`, `prepare_paper_interactions.py`, `prepare_paper_render_sync.py`, `prepare_paper_listener_pack.py`, `prepare_paper_craftengine.py`, and `prepare_paper_schematic.py` from `scripts/`, in that order. These generate the item models, editing channels, render synchronization and listener-pack playback. The generators understand the independent timers and flush-before-reload flow; a preparation test verifies feature presence and repeatability. The generated source belongs in a disposable build checkout, not a commit over the base source. Running Gradle alone on the base source produces an incomplete plugin.
