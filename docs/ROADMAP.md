...
[返回首页](../README.md) · [文档中心](README.md) · [2.13.1 更新说明](releases/2.13.1.md)

当前基线为 Full Fabric / Paper Client **2.13.1** 与 Paper Server **0.14.1**。

## 原版客户端自动资源包

- [x] 将物品资源、32 种代表音色 × 22 个半八度锚点及 47 个打击乐采样组合为可自动下发的资源包。
- [x] 从 CraftEngine 获取生成包及托管地址，校验 SHA-1 后统一发送；服务端按加载状态切换自定义声音与原版回退。
...
CI 启动实际 Paper Client JAR，等待资源加载，检查内置物品包、GUI、原版 Registry、全部 MIDI pitch 和原版声音行为，并用实际投影导出器生成文件后再读回验证。纯 Java 测试覆盖协议、参数边界、批次顺序、坐标变换。
