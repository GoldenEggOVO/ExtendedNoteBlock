...
**当前正式版：Full Fabric / Paper Client 2.8.0，Paper Server 0.8.1，目标 Minecraft 26.2。**

[下载正式版](https://github.com/GoldenEggOVO/ExtendedNoteBlock/releases/tag/v2.8.0-mc26.2) · [2.8.0 更新说明](docs/releases/2.8.0.md) · [安装指南](docs/INSTALLATION.md) · [日本語](docs/README_ja-jp.md)
...
2. 将 `ExtendedNoteBlock-Full-Fabric-2.8.0-mc26.2.jar` 放入客户端 `mods/`。
3. Fabric 多人服务器还需安装同版 Full Fabric 和 Fabric API。

### Paper / Purpur 服务器

1. 将 `ExtendedNoteBlock-Paper-Server-0.8.1-mc26.2.jar` 放入服务器 `plugins/`，重启服务器。
2. 需要完整音色、编辑界面和世界方块外观的玩家，在 Fabric 26.2 客户端安装 Fabric API 与 `ExtendedNoteBlock-Paper-Client-Fabric-2.8.0-mc26.2.jar`。
...
| --- | --- | --- |
| MIDI 0–127、乐器、力度、延音、延迟、淡入淡出 | 支持 | 支持；2.8.0 的低音修复待游戏内试听确认 |
...
**2.8.0 通过了编译、CI 和静态检查；这些结果不等于 Purpur 游戏内验证。** Paper-safe Litematic 解决了未知方块变空气的问题，粘贴后的原版载体还不会自动恢复为可播放的 ENB 对象。
